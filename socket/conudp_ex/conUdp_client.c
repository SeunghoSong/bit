...
int main(int argc,char* argv[]){
    ...
    server_addr.sin_port=htons(atoi(argv[2]));//문자열로 된 포트번호를 숫자로 변경후 htons 함수 short 형으로 변형
     ...
     // connected UDP는 connect를 추가 연결할 장소를 정해 놓는다. (연결형은 아님)
    connect(sock,(struct sockaddr*)&serv_addr,sizeof(server_addr));
    ...
    while(1)
    {
        ...
        message[str_len]=0;
        /* conncted udp를 사용하는 이유
         sendto ,recvfrom을 사용했을때 전송할수 있는 데이터가 한정이 되버림
        connect() 를 사용하면 write read 함수를 사용가능 물론 fread fwrite도 가능                                                                                                                                                               
        */
